...
// Demonstrates @tola/current source field for Issue #40
...
#import "@tola/current:0.0.0": headings, path, source

/// Parse date from filename with format: YYYY_MM_DD_slug.typ
/// Returns: (date: datetime or none, slug: str)
#let parse-dated-filename(source-path) = {
  if source-path == none { return (date: none, slug: none) }

  let filename = source-path.split("/").last()
...
/// Generate permalink from source path
/// posts/2025_02_25_hello.typ -> /posts/hello/
#let derive-permalink(source-path, slug) = {
  if source-path == none or slug == none { return none }
  let dir = source-path.split("/").slice(0, -1).join("/")
  if dir == "" { "/" + slug + "/" } else { "/" + dir + "/" + slug + "/" }
}

#let dated-post = wrap-page(
  base: base,

  transform-meta: m => {
    let parsed = parse-dated-filename(source)
    ...
    if "permalink" not in m and parsed.slug != none {
      m.insert("permalink", derive-permalink(source, parsed.slug))
    }
    m
  },
  ...
  view: (body, m) => {
    ...
    // Debug info showing source parsing
    let debug-view = html.div(class: "my-4 p-4 bg-surface/50 rounded text-sm font-mono")[
      #html.div(class: "text-cyan-400 mb-2")[Debug: \@tola/current.source]
      #html.div()[source: #source]
      #html.div()[path: #path]
